...
EDITED = 'Eira Chae <vine9151@gmail.com>'
VERSION = '0.3'
...
def mk_rotate_node(output, name, command):
    """ Function to Make Rotate Node """

    right_rotated = "0 1 0 -1 0 1 0 0 1"
    left_rotated = "0 -1 1 1 0 0 0 0 1"
    invert_rotated = "-1 0 1 0 -1 1 0 0 1"
    normal_rotated = "1 0 0 0 1 0 0 0 1"

    rotated = []
    rotated.append('xinput set-prop \"04F31234:00 04F3:2C64\" --type=float \"Coordinate Transformation Matrix\" ')
    rotated.append('sleep 0.5')
    rotated.append('xinput set-prop \"04F31234:00 04F3:2C64 Stylus Pen (0)\" --type=float \"Coordinate Transformation Matrix\" ')
    rotated.append('sleep 0.5')
    rotated.append('xinput set-prop \"04F31234:00 04F3:2C64 Stylus Eraser (0)\" --type=float \"Coordinate Transformation Matrix\" ')

    if name == "right":
        rotated[0] += right_rotated
        rotated[2] += right_rotated
        rotated[4] += right_rotated
    elif name == "left":
        rotated[0] += left_rotated
        rotated[2] += left_rotated
        rotated[4] += left_rotated
    elif name == "inverted":
        rotated[0] += invert_rotated
        rotated[2] += invert_rotated
        rotated[4] += invert_rotated
    else:
        rotated[0] += normal_rotated
        rotated[2] += normal_rotated
        rotated[4] += normal_rotated
        ...
        if ' current' in i:
            ...
            for action in actions:
                if not action:
                    ...
                    node.append(mk_rotate_node(output, *action))
